...
from modules.analytics.formal.correlation_regression import (
    regression_vulnerability_prediction,
    vulnerability_correlation_analysis,
)
from modules.analytics.formal.descriptive import (
    get_descriptive_stats,
    get_raw_vulnerabilities,
)
from modules.analytics.formal.pareto_80_20 import pareto_vulnerability_analysis
from modules.analytics.formal.probability_fitting_distribution import (
    vulnerability_distribution_analysis,
)
...
from modules.analytics.vulnerability_analysis import (
    analyze_results,
    create_priority_matrix,
    generate_summary_stats,
)
from modules.db.database import Database
from modules.db.table_collection import Scan, ScheduledScans
...
from modules.utils.__utils__ import (
    check_directories,
    check_url_local_test,
    run_start_scan,
)
...
async def wapiti_scan(request: ScanRequest) -> dict:
    ...
    if _whatweb_results.__contains__("error"):
        _db.insert_wapiti_quick_report(
            _scan_start, _whatweb_results["message"], result, scan_time, _URL
        )
        scan_tracker.advance_step(session, ScanStep.SUCCESS)
        return {
            "data": result,
            "plugins": {"fingerprinted": _whatweb_results, "patchable": _query_results},
            "scan_time": scan_time,
        }
    else:
        _db.insert_wapiti_quick_report(
            _scan_start, _whatweb_results["data"], result, scan_time, _URL
        )
        scan_tracker.advance_step(session, ScanStep.SUCCESS)
        return {
            "data": result,
...
async def zap_passive_scan(request: ScanRequest) -> dict:
    ...
    if query_result.__contains__("error"):
        _db.insert_zap_report(
            _scan_start, raw_whatweb_result, zap_result, scan_time, _URL
        )
        scan_tracker.advance_step(session, ScanStep.SUCCESS)
        return {
            "data": zap_result,
        ...
        _db.insert_zap_report(
            _scan_start, raw_whatweb_result["data"], zap_result, scan_time, _URL
        )
        scan_tracker.advance_step(session, ScanStep.SUCCESS)
        return {
            "data": zap_result,
...
async def scan(request: ScanRequest) -> dict:
    ...
    _results = analyze_results(
        session, wapiti_result, zap_result, raw_whatweb_result, query_result
    )
    ...
    # Save report in disk
    scan_tracker.advance_step(session, ScanStep.SAVING)
    report_path = os.path.join(full_scan_path, f"{session}.json")
...
async def poll_data_summary(
    days: int, target: str = Query(None, description="Filter by target domain")
):
...
async def get_vulnerabilities_list(
    target: str = Query(None), start: str = Query(None), end: str = Query(None)
):
    """
    ...
    try:
        with Session(_db.engine) as session:
            return get_raw_vulnerabilities(
                session, target_domain=target, start_date=start, end_date=end
            )
    except Exception as e:
    ...
    try:
        with Session(_db.engine) as session:
            ...
            stats = get_descriptive_stats(
                session, mode=mode, target_domain=target, start_date=start, end_date=end
            )
            return stats
    except Exception as e:
...
async def poll_data_correlation(
    target: str = Query(None, description="Filter by target domain"),
):
...
async def poll_data_distribution(
    ...
    try:
        return vulnerability_distribution_analysis(
            report_id=report_id, target_domain=target
        )
    except Exception as e:
...
async def poll_data_regression(
    target: str = Query(None, description="Filter by target domain"),
):
...
async def get_schedule(schedule_id: str):
    ...
    try:
        with Session(_db.engine) as session:
            schedule = (
                session.query(ScheduledScans)
                .filter(ScheduledScans.id == schedule_id)
                .first()
            )
...
async def update_schedule(schedule_id: str, schedule: ScheduleRequest):
    ...
    try:
        ...
        if schedule.job_type not in ["interval", "cron"]:
            raise HTTPException(
                status_code=400, detail="job_type must be 'interval' or 'cron'"
            )
        ...
        return {"message": "Schedule updated successfully", "schedule_id": schedule_id}
    except HTTPException:
        raise
    except Exception as e:
    ...
    try:
        with Session(_db.engine) as session:
            ...
            return {
                "schedules": result,
                "count": len(result),
                "filters": {"name": name, "target": target, "job_type": job_type},
            }
    except HTTPException:
        raise
    except Exception as e:
...
async def delete_history_report(report_id: str):
    ...
    try:
        ...
        )  # Continuation of main.py - Add these routes after the scan endpoint
