...
class Database:
    ...
    def insert_wapiti_quick_report(self, timestamp: datetime, plugins: list, raw_data: dict,
        ...
        with Session(engine) as session:
            ...
            self._insert_wapiti_vulnerabilities(
                report_id, timestamp, raw_data, session)
            session.commit()
            return report_id
    ...
    def insert_scan_report(self, timestamp: datetime, plugins: list,
        ...
        _plugins_dump = json.dumps(plugins)
        total_union = sum(len(scanner_results)
                          for scanner_results in analytics_data["union"])
        ...
        with Session(engine) as session:
            ...
            if summary_stats:
                ...
                agreement_str = summary_stats.get(
                    "scanner_agreement_rate", "0%")
                confidence_str = summary_stats.get("confidence_rate", "0%")

                report.scanner_agreement_rate = float(
                    agreement_str.rstrip('%')) if agreement_str else 0.0
                report.confidence_rate = float(
                    confidence_str.rstrip('%')) if confidence_str else 0.0
            ...
            if ai_summary:
                report.ai_summary_vulnerabilities = ai_summary.get(
                    "summary", {}).get("vulnerabilities", "")
                report.ai_summary_tech = ai_summary.get(
                    "summary", {}).get("tech", "")
            ...
            try:
                ...
                self._insert_wapiti_vulnerabilities(
                    report_id, timestamp, wapiti_raw_data, session, analytics_data.get("intersection", []))
        ...
        for vulnerability in raw_data["runs"][0]["results"]:
            ...
            _vuln = Vulnerability(
                ...
                http_request=json.dumps(
                    vulnerability["properties"]["har"]) if vulnerability["properties"]["har"] is not None else None,
    ...
    @staticmethod
    def _insert_wapiti_vulnerabilities(parent_report_id, scan_time, raw_data, session, intersection_list=None):
        _entries = []
        _rules = utils.unroll_sarif_rules(raw_data)
